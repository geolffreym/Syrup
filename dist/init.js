	...
	(function (windowGlobal) {
	  ...
	  */
		_$_.add('$', function (dom) {
			...
			if (_.isHtml(dom)) {
				...
				_tmp.innerHTML = dom;
				_self.collection = _tmp.children.length > 1 ? _tmp.children : _tmp.firstChild;
			} else {
				_self.collection = !_.isObject(dom) && _.isString(dom) ? dom.indexOf('+') > -1 ? document.querySelectorAll(_.replace(dom, '+', _.emptyStr)) : document.querySelector(dom) : dom;
			}
	  ...
	  */
		_$_.add('ready', function (callback) {
			if (_.isGlobal(this.collection)) this.collection.addEventListener("DOMContentLoaded", callback.bind(this));
			return this;
		});
	  ...
	  */
		_$_.add('filter', function (filter, callback, e_handler) {
			//Not string.. pass!!
			if (!_.isString(filter)) return this;
			...
			this.each(function (elem) {
				...
				if (elem.is(filter)) {
					// Push on list
					if (!_.isFunction(callback)) _result.push(elem);
			...
			//Output
			return _.isFunction(callback) && this || _.specArray(_result);
		});
	  ...
	  */
		_$_.add('data', function (name, value) {
			...
			return _.isString(value) && _.isNumber(value) && this || _.specArray(_values);
		});
	  ...
	  */
		_$_.add('prop', function (_prop) {
			...
			return _.isString(_prop) && _.specArray(_props) || this;
		});
	  ...
	  */
		_$_.add('attr', function (attr) {
			...
			return _.isString(attr) && _.specArray(_attr) || this;
		});
	  ...
	  */
		_$_.add('css', function (css) {
			...
			_self.each(function (dom) {
				...
				//String?
				if (_.isString(css)) {
					_css.push(windowGlobal.getComputedStyle(dom, null).getPropertyValue(css));
			...
			return _.isString(css) && _.specArray(_css) || this;
		});
	  ...
	  */
		_$_.add('html', function (html) {
			...
			//Is Html?
			return (_.isHtml(html) || _.isString(html)) && this.prop({ 'innerHTML': html }) && this || this.prop('innerHTML');
		});
	  ...
	  */
		_$_.add('text', function (text) {
			...
			//Number or string?
			return _.isString(text) && this.prop({ 'textContent': text }) && this || this.prop('textContent');
		});
	  ...
	  */
		_$_.add('val', function (text) {
			...
			//Number or string?
			return _.isString(text) && this.prop({ 'value': text }) && this || this.prop('value');
		});
	  ...
	  */
		_$_.add('parent', function (callback) {
			...
			this.each(function (_elem) {
				if (_elem.parentNode && !_.isGlobal(_elem.parentNode)) {
					//The parent
					_parent = _$(_elem.parentNode);

					// Push on list
					if (!_.isFunction(callback)) _result.push(_parent);

					//Callback if needed
					_.isFunction(callback) && _.callbackAudit(callback, _parent);
				}
			});
				...
				if (_elem.children.length > 0) {
					_.each(_elem.children, function (v, i) {
						if (_.isNumber(i)) {
							...
							// Push on list
							if (!_.isFunction(callback)) _result.push(_child);

							//Callback if needed
							_.callbackAudit(callback, _child);
						}
					});
				}
			});
		...
		_$_.add('next', function (callback) {
			...
			this.each(function (_elem) {
				if (_elem.nextElementSibling) {
					...
					// Push on list
					if (!_.isFunction(callback)) _result.push(_sibling);
	  ...
	  */
		_$_.add('nexts', function (filter, callback) {
			...
			this.next(function (elem) {
				...
				} while (_sibling.get(0).nextElementSibling && (_sibling = _$(_sibling.get(0).nextElementSibling)).exist);
			});
		...
		_$_.add('find', function (filter, callback, find) {
			...
			this.children(function (elem) {
				if (elem.is(filter)) {
					...
					//Callback if needed
					_.isFunction(callback) && _.callbackAudit(callback, elem, filter);
				} else {
					// Keep searching
	  ...
	  */
		_$_.add('parents', function (parent_class, callback, parents) {
			//Not string.. pass!!
			if (!_.isString(parent_class)) return this;
	  ...
	  */
		_$_.add('hasClass', function (cls) {
			...
			return elem.classList && Array.prototype.indexOf.call(elem.classList, cls || _.emptyStr) > -1;
		});
			...
			return this.each(function (elem) {
				if (_$(elem).hasClass(cls)) {
					if (elem.classList) {
						...
						elem.className = _.replace(elem.className, _.toRegExp(cls, 'g'), _.emptyStr);
					}
				}
			});
		});
	  ...
	  * */
		_$_.add('is', function (context) {
			...
			var _dom = this.get(0),
			    _match = _dom.matchesSelector || _dom.webkitMatchesSelector || _dom.mozMatchesSelector || _dom.oMatchesSelector || _dom.msMatchesSelector;

			return context in _dom || _dom['type'] === context || _match.call(_dom, context);
		});
	  ...
	  * */
		_$_.add('get', function (find) {
			return _.objectAsString(this.collection) == '[object NodeList]' && find in this.collection && this.collection[find] || this.collection;
		});
	  ...
	  */
		_$_.add('each', function (callback) {
			...
			//Child nodes
			//Global nodes?
			if ('childNodes' in _element || _.isGlobal(_element)) {
		...
		_$_.add('offset', function (_object) {
			...
			this.each(function (elem) {
				...
				if (_.isObject(_object)) {
					...
					if (_.isSet(_object.top)) elem.style.top = _.isNumber(_object.top) ? _object.top + 'px' : _object.top;

					if (_.isSet(_object.left)) elem.style.left = _.isNumber(_object.left) ? _object.left + 'px' : _object.left;

					if (_.isSet(_object.bottom)) elem.style.bottom = _.isNumber(_object.bottom) ? _object.bottom + 'px' : _object.bottom;

					if (_.isSet(_object.right)) elem.style.right = _.isNumber(_object.right) ? _object.right + 'px' : _object.right;
				} else {
					// To get?
					//Find position
			...
			//Result or reference
			return _.isObject(_object) && this || _.specArray(_offset);
		});
		...
		_$_.add('animate', function (prop, conf, callback) {
			...
			return this.each(function (elem) {
				if (_.isSet(elem.animate)) {
					...
					if (_.isFunction(conf)) callback = conf;

					conf = !_.isObject(conf) && !_.isNumber(conf) ? {} : conf;

					conf.iterations = _.isSet(conf.iterations) ? conf.iterations : 1;

					conf.duration = _.isSet(conf.duration) ? conf.duration : 1000;

					conf.delay = _.isSet(conf.delay) ? conf.delay : 300;
	  ...
	  */
		Syrup.add('isObject', function (obj) {
			return _.objectAsString(obj) === '[object Object]' || (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object' && _.objectAsString(obj) !== '[object Null]';
		});
	  ...
	  */
		Syrup.add('isGlobal', function (obj) {
			return _.objectAsString(obj) === "[object global]" || _.objectAsString(obj) === "[object Window]" || _.objectAsString(obj) === "[object HTMLDocument]" || _.objectAsString(obj) === "[object Document]";
		});
	  ...
	  */
		Syrup.add('isEmpty', function (input) {
			if (_.isObject(input)) return _.getObjectSize(input) == 0;

			if (_.isArray(input)) return input.length === 0;

			return !input || input === _.emptyStr || /^\s+$/.test(input);
		});
	  ...
	  */
		Syrup.add('warning', function (msg, breakpoint) {
			console.log(msg + (breakpoint ? ' | Method: ' + breakpoint : _.emptyStr));
		});
	  ...
	  */
		Syrup.add('error', function (msg, breakpoint) {
			throw msg + (breakpoint ? ' | Method: ' + breakpoint : _.emptyStr);
		});
	  ...
	  */
		Syrup.add('replace', function (_string, _find, _replace) {
			...
			if (_.isRegExp(_find) && _.isObject(_replace)) {
				...
				//Is String?
				return _string.replace(_find, _replace);
			}
		});
	  ...
	  */
		Syrup.add('getDate', function (fecha) {
			...
			_fecha = _.isSet(fecha) ? new Date(fecha) : _fecha;

			if (_fecha === 'Invalid Date') {
				_.error(_fecha);
			}

			dia_ = dia_ < 0xA ? '0' + dia_ : dia_;

			meridiano_ = hora_ > 0xC ? 'PM' : 'AM';

			hora_ = hora_ > 0xC ? hora_ - 0xC === 0 ? 0xC : hora_ - 0xC : hora_ < 0xA ? '0' + hora_ : hora_;

			minutos_ = minutos_ < 0xA ? '0' + minutos_ : minutos_;

			segundos_ = segundos_ < 0xA ? '0' + segundos_ : segundos_;
	  ...
	  */
		Syrup.add('objectImmutable', function (obj) {
			if (_.isObject(obj)) return Object.freeze(obj);
	  ...
	  */
		Syrup.add('objectWatch', function (obj, callback, conf) {
			if (_.isObject(obj) && _.isSet(callback)) Object.observe(obj, callback, conf ? conf : []);
	  ...
	  */
		Syrup.add('requestAnimationFrame', function (callback) {
			return (windowGlobal.requestAnimationFrame || windowGlobal.webkitRequestAnimationFrame || windowGlobal.mozRequestAnimationFrame || function (call) {
	  ...
	  * **/
		Syrup.add('oChars', function (string, find) {
			if (_.isString(string)) return string.split(find).length - 1;
			return 0;
		});
	  ...
	  */
		Syrup.add('jsonToQueryString', function (_object) {
			var _return = _.emptyStr,
			    _size = _.isObject(_object) ? _.getObjectSize(_object) : 0;
			...
			return _return.lastIndexOf('&') > -1 ? _return.slice(0, -1) : _return;
		});
		...
		Syrup.add('queryStringToJson', function (_string) {
			...
			if (_.isString(_string)) {
				...
				_.each(_string, function (value) {
					value = value.split('=');
					if (!_.isEmpty(value[0])) _return[value[0]] = value[1] || _.emptyStr;
				});
			}
	  ...
	  */
		Syrup.add('each', function (_object, callback) {
			...
			if (_.isObject(_object)) _keys = _.getObjectKeys(_object);
			...
			while (_i++ < _max) {
				...
				//Filter function ?
				_.callbackAudit(callback.bind(_p), _keys && _object[_keys[_i - 1]] || _object[_i - 1], _keys && _keys[_i - 1] || _i - 1);
	  ...
	  */
		Syrup.add('specArray', function (arr) {
			...
			return arr.length > 1 ? arr : 0 in arr ? arr[0] : null;
		});
	  ...
	  */
		Syrup.add('replaceInArray', function (find, replace, haystack) {
			...
			if (this.matchInArray(haystack, [find])) {
				_.each(haystack, function (v, i) {
					if (_.isString(v)) haystack[i] = _.replace(v, find, replace);
				});
			}
	  ...
	  * */
		...
		Syrup.add('toRegExp', function (element, det) {
			if (_.isString(element)) return new RegExp(element, det || 'g');
		});
	  ...
	  */
		Syrup.add('toObject', function (element, element2) {
			...
			if (_.isJson(element)) return JSON.parse(element);

			if (_.isString(element) || _.isNumber(element)) return nativeObject.valueOf.call(element);

			if (!_.isArray(element)) _.error(WARNING_SYRUP.ERROR.NOARRAY, '(Syrup .toObject)');
	  ...
	  */
		Syrup.add('extend', function (target, source, overwrite) {
			...
			_.each(source, function (v, i) {
				if (!target.hasOwnProperty(i) || _.isSet(overwrite)) {
					target[i] = v;
				}
			});
			return target;
		...
		_.nav = {};
		_.nav.unsupported = !windowGlobal.localStorage || !windowGlobal.File || !windowGlobal.FileReader || !windowGlobal.FileList || !windowGlobal.Blob || !windowGlobal.Worker || !windowGlobal.WebSocket;
