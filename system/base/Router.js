...
(function (window) {
	...
	Router.add ('_route', function (route_name) {
		...
		return (new RegExp ('^' + _the_route + '$').test (_uri_path))
			? { uri: _uri_path, route: _the_route, query: _query_params } : false;
	});
	...
	Router.add ('when', function (route_name, conf) {
		...
		if ( (_router = _self._route (route_name)) ) {
			//No default
			_self.default = false;
			_self.redirect (route_name, _router.query);
		}
	...
	Router.add ('redirect', function (route_name, params, config) {
		...
		return (new Promise (function (resolve, reject) {
			...
			//Rewrite rules or remove regexp params
			_the_new_route = _self._rewriteRules (_the_new_route, _params) ||
							 _.replace (_the_new_route, _self.findParams, _.emptyStr);
